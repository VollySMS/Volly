...
companyAuthRouter.post('/company-signup', jsonParser, (request, response, next) => {
  if(!request.body.accountName || !request.body.password || !request.body.email) {
    return next(new httpErrors(400, '__ERROR__ <accountName>, <email>, and <password> are required to sign up.'));
  }

  return Company.create(request.body.accountName, request.body.password, request.body.email)
